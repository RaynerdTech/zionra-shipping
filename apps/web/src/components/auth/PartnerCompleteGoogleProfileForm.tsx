 ...
 * to create the Zionra shipping-partner account and onboarding session.
 */
...
function ReferralSourceSelect({
  ...
  return (
    <div ref={containerRef} className="relative min-w-0">
      <button
        ref={triggerRef}
        id="partnerGoogleReferralSource"
        type="button"
        aria-haspopup="listbox"
        aria-expanded={isOpen}
        aria-controls="partnerGoogleReferralSourceOptions"
      ...
      {isOpen ? (
        <div
          id="partnerGoogleReferralSourceOptions"
          role="listbox"
...
export default function PartnerCompleteGoogleProfileForm() {
    ...
    async function loadPendingProfile() {
      try {
        const response = await fetch(
          buildApiUrl(routes.api.partnerAuth.googlePendingProfile),
          {
            method: "GET",
            credentials: "include",
            cache: "no-store",
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const response = await fetch(
        buildApiUrl(routes.api.partnerAuth.googleCompleteProfile),
        {
          method: "POST",
          credentials: "include",
          headers: {
      ...
      if (!response.ok) {
        ...
        setErrors({
          ...(result.errors ?? {}),
          form: getApiMessage(result, "Unable to complete your partner account."),
        } as FormErrors);
        return;
      }
  ...
  function restartGoogleSignup() {
    ...
    window.location.replace(buildApiUrl(routes.api.partnerAuth.google));
  }
        ...
        <div className="relative z-[1] mx-auto w-full max-w-[520px]">
          ...
          <header className="mt-6 text-center">
            ...
            <p className="mx-auto mt-2 max-w-[390px] font-sans text-sm font-normal leading-[22px] text-text-body-light md:text-base md:leading-[26px]">
              Add the remaining details to finish setting up your Zionra shipping-partner
              account.
            </p>
          ...
          {isLoadingProfile ? (
            ...
            <div className="mt-8 rounded-xl border border-primary-02 bg-primary-01 px-5 py-6 text-center">
              ...
              >
                Return to partner registration
              </Link>
            </div>
          ) : profile ? (
            <>
                ...
                <div className="mt-4 grid grid-cols-1 gap-5 md:grid-cols-2 md:gap-x-6">
                  ...
                  <div className="min-w-0">
                    ...
                    <ReferralSourceSelect
                      ...
                      onChange={(value) =>
                        updateValue("referralSource", value)
                      }
                    />
                  </div>
                </div>
