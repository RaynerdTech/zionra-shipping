 ...
 * It controls mutually exclusive card toggles and routes each account action.
 */
...
function DecorativeCircles() {
  return (
    <div
      aria-hidden="true"
      className="pointer-events-none absolute md:right-[0px] md:-top-[56px] top-[-34px] right-[-6px] z-10 md:h-[240px] md:w-[240px] h-[136px] w-[136px]"
    >
    <svg
  xmlns="http://www.w3.org/2000/svg"
...
export default function AccountTypeSelector() {
  const [openType, setOpenType] = useState<AccountType | null>(null);

  function toggleType(type: AccountType) {
    setOpenType((currentType) => (currentType === type ? null : type));
  }
  ...
  return (
    <main className="relative min-h-screen overflow-hidden  pt-6 md:flex md:items-center md:justify-center bg-neutral-01 md:p-8">
      <DecorativeCircles />

      <section
        className={`relative mx-auto flex min-h-[calc(100dvh-24px)] w-full flex-col overflow-hidden rounded-[16px] bg-white px-6 pb-6 pt-4 md:m-0 md:w-[840px] md:max-w-full md:px-9 md:pb-6 md:pt-11 ${
          openType
      ...
      >
        ...
        <div className="mx-auto grid w-full max-w-[700px] grid-cols-1 items-start gap-10 md:grid-cols-2 md:gap-7">
          <div className="min-w-0">
            <button
              type="button"
              className={`${baseCardClasses} hover:border-primary-06 ${
                openType === "customer" ? "border-primary-06" : ""
              }`}
              onClick={() => toggleType("customer")}
              aria-expanded={openType === "customer"}
          ...
          <div className="min-w-0">
            <button
              type="button"
              className={`${baseCardClasses} hover:border-secondary-06 ${
                openType === "partner" ? "border-secondary-06" : ""
              }`}
              onClick={() => toggleType("partner")}
              aria-expanded={openType === "partner"}
              aria-controls="partner-actions"
            >
              <span className="inline-flex h-12 w-12 shrink-0 items-center justify-center rounded-lg bg-neutral-02 md:h-[54px] md:w-[54px] md:rounded-[9px]">
                <ShippingPartnerIcon />
              </span>
