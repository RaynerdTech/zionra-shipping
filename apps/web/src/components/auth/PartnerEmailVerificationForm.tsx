/**
 * Responsibility:
 * Renders and submits the shipping-partner email-verification flow.
...
export default function PartnerEmailVerificationForm({
  ...
  async function handleVerify(event: FormEvent<HTMLFormElement>) {
    ...
    if (!hasEmail) {
      ...
      setFeedback({
        type: "error",
        message: "Return to partner registration and submit your email again.",
      });
      return;
    }
    ...
    try {
      const response = await fetch(
        buildApiUrl(routes.api.partnerAuth.verifyEmail),
        {
          method: "POST",
          credentials: "include",
          headers: {
  ...
  async function handleResend() {
    ...
    try {
      const response = await fetch(
        buildApiUrl(routes.api.partnerAuth.resendVerificationCode),
        {
          method: "POST",
          credentials: "include",
          headers: {
