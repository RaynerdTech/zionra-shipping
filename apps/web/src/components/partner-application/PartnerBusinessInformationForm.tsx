          ...
          {values.contacts.map((contact, index) => {
            ...
            return (
              <section key={contact.id ?? `contact-${index}`} className={index ? "rounded-xl border border-neutral-02 p-4" : ""}>
                {index ? <div className="mb-4 flex items-center justify-between"><h3 className="font-display text-base font-semibold text-primary-10">Additional Contact {index}</h3><button type="button" onClick={() => removeContact(index)} className="font-sans text-sm text-error hover:underline">Remove</button></div> : null}
